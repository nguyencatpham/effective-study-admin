...
@Component({
  ...
  selector: 'topics-main',
  // Our list of styles in our component. We may add more to compose many styles together
  encapsulation: ViewEncapsulation.None,
  // Every Angular template is first compiled by the browser before Angular runs it's compiler
  templateUrl: 'topics-main.template.html',
  styleUrls: [
    'topics-main.style.scss'
  ]
})
export class TopicsMainComponent implements OnInit,
  ...
  public onActivate(event: RowSelectEvent): void {
    if (event.column.name) {
      ...
      //   enableBackView: true
      // });
      this._router.navigate(['topics-management', event.row.id]);
    }
  ...
  public refreshDatatable(draw?: number): void {
    ...
    this._topicsManagementService.getListTopics(params)
      .subscribe((resp: any): void => {
        // Stop loading from linear loading bar of ngx-datatable
        this.tableConfig.loading = false;
        // this.topicData.totalRecord = resp.totalRecords;
        // this.topicData.data = resp.records;

        this.topicData.totalRecord = resp.length;
        this.topicData.data = resp;
  ...
  public addNewTopic(): void {
    this._router.navigate(['topics-management', 'new']);
  }
}
