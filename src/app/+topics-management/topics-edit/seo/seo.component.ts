...
export class TopicEditSeoComponent implements OnInit {
  ...
  public ngOnInit(): void {
    ...
    this._activatedRoute.parent.params.subscribe((params: {id: number}) => {
      if(params.id) {
        ...
        let dt: URLSearchParams = new URLSearchParams();
        dt.set('entityId', params.id.toString());
        dt.set('entityName', 'topics');

        this._topicsManagementService.getSeo(dt).subscribe(
          (resp: any): void => {
            this.seo = resp;
  ...
  public onSubmitForm(): void {
    ...
    if (this.seo && this.seo.id) {
      this._seoService.updateSeo(this.seo.id, model).subscribe(
        (resp: BasicResponse) => {
          this._toastrService.success("Topic SEO is updated", 'Success');
        },
        (err) => {
          this._toastrService.error(err, 'Error');
        });
      ...
      this._seoService.createSeo(model).subscribe(
        (resp: SeoInfo) => {
          this.seo = resp;
          this.frm.patchValue(this.seo);
          this._toastrService.success("Topic SEO is added", 'Success');
        },
        (err) => {
          this._toastrService.error(err, 'Error');
        });
      ...
      if(resp) {
        this._seoService.deleteSeo(this.seo.id).subscribe(
          resp => {
            this.seo = null;
            this.buildForm();
            this._toastrService.success("Topic SEO is deleted", "Success");
          },
          err => {
            this._toastrService.error(err, "Error");
          }
