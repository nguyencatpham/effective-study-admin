...
export class CategoriesMainComponent implements OnInit,
  ...
  public ngOnInit(): void {
    ...
    this._breadcrumbService
      .hideRoute('/categories-management/.*');
  }
  ...
  public onActivate(event: RowSelectEvent): void {
    if (event.column.name) {
      ...
      //   enableBackView: true
      // });
      console.log("CateEvent: " + event.row.id);
      this._router.navigate(['categories-management', event.row.id, 'edit']);
    }
  ...
  public refreshDatatable(draw?: number): void {
    ...
    this._categoriesManagementService.getListCategories(params)
      .subscribe((resp: any): void => {
        // Stop loading from linear loading bar of ngx-datatable
        this.tableConfig.loading = false;
        this.categoryData.totalRecord = resp.totalRecords;
        this.categoryData.data = resp.records;
  ...
  public addNewCategory(): void {
    this._router.navigate(['categories-management', 'new']);
  }
}
