...
@Component({
  ...
  templateUrl: 'orders-main.template.html',
  styleUrls: [
    'orders-main.style.scss'
  ]
})
export class OrdersMainComponent implements OnInit,
  ...
  constructor(private _router: Router,
              private _ordersManagementService: OrdersManagementService,
  ...
  public onActivate(event: RowSelectEvent): void {
    if (event.column.name) {
      ...
      //   enableBackView: true
      // });
      this._router.navigate(['orders-management', event.row.id]);
    }
  ...
  public refreshDatatable(draw?: number): void {
    ...
    this._ordersManagementService.getListOrders(params)
      .subscribe((resp: any): void => {
        // Stop loading from linear loading bar of ngx-datatable
        this.tableConfig.loading = false;
        this.orderData.totalRecord = resp.totalRecords;
        this.orderData.data = resp.records;
  ...
  public addNewOrder(): void {
    this._router.navigate(['orders-management', 'new']);
  }
}
