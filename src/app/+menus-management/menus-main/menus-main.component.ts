...
@Component({
  ...
  templateUrl: 'menus-main.template.html',
  styleUrls: [
    'menus-main.style.scss'
  ]
})
export class MenusMainComponent implements OnInit,
  AfterViewChecked {
  ...
  public onActivate(event: RowSelectEvent): void {
    if (event.column.name) {
      ...
      //   enableBackView: true
      // });
      this._router.navigate(['menus-management', event.row.id]);
    }
  ...
  public refreshDatatable(draw?: number): void {
    ...
    this._menusManagementService.getListMenu(params)
      .subscribe((resp: any): void => {
        // Stop loading from linear loading bar of ngx-datatable
        this.tableConfig.loading = false;
        this.menuData.totalRecord = resp.totalRecords;
        this.menuData.data = resp.records;
  ...
  public addNewMenu(): void {
    this._router.navigate(['menus-management', 'new']);
  }
}
