...
@Component({
  ...
  selector: 'shippings-main',
  // Our list of styles in our component. We may add more to compose many styles together
  encapsulation: ViewEncapsulation.None,
  // Every Angular template is first compiled by the browser before Angular runs it's compiler
  templateUrl: 'shippings-main.template.html',
  styleUrls: [
    'shippings-main.style.scss'
  ]
})
export class ShippingsMainComponent implements OnInit,
  AfterViewChecked {
  ...
  public ngOnInit(): void {
    ...
    this._breadcrumbService
      .hideRoute('/shippings-management/.*');
  }
  ...
  public onActivate(event: RowSelectEvent): void {
    if (event.column.name) {
      ...
      //   enableBackView: true
      // });
      this._router.navigate(['shippings-management', event.row.id]);
    }
  ...
  public refreshDatatable(draw?: number): void {
    ...
    this._shippingsManagementService.getListShippings(params)
      .subscribe((resp: any): void => {
        // Stop loading from linear loading bar of ngx-datatable
        this.tableConfig.loading = false;
        this.shippingData.totalRecord = resp.totalRecords;
        this.shippingData.data = resp.records;
   ...
   * add new shipping
   */
  public addNewShipping(): void {
    this._router.navigate(['shippings-management', 'new']);
  }
}
